...
1. 创建 `.md` 文件到对应专栏目录，遵循已有编号/命名约定
2. 同步更新专栏 `index.md` 中的表格条目
3. 侧边栏自动扫描生成，无需手动配置

### 文章配图

1. 图片存放路径：`images/{文章英文名}/{图片文件}`
2. 微信 CDN 图片 URL 后缀 `.png` 但实际是 WebP 格式，需 `convert` 转为真 PNG
3. 引用路径使用相对路径：`images/{文章英文名}/{图片文件}.png`
