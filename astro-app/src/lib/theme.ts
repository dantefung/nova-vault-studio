...
export const COOKIE_NAME = 'landingTheme'
 ...
 * Astro server-side 没有 localStorage，需要客户端把选择写到 cookie，
 * 下一次请求 SSR 才能拿到正确风格。
 */
...
(function(){
  try {
    ...
    var cv = document.cookie.match(/(?:^|; )${COOKIE_NAME}=([^;]+)/);
    var current = cv ? decodeURIComponent(cv[1]) : null;
    if (ls && ls !== current) {
