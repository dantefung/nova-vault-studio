...
![LightRAG 知识召回排查路径](images/01-flowchart-troubleshooting.jpg)
...
![LightRAG 查询处理流程](images/02-flowchart-query-pipeline.jpg)
...
![LightRAG 双路并行互补架构](images/03-framework-dual-path.jpg)
...
- **通过 `references` 验证**：最新 API 在返回最终答案的同时会返回 `references`，可以清晰看到最终答案融合了哪些图谱关系和文本段落

> LightRAG 通过"先并行检索、后融合构建"的内部机制，将图谱的**关联知识**与原始文档的**文本证据**无缝整合到同一个提示上下文中，提供给大语言模型进行回答。
