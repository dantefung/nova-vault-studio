---
title: "Prompt 高级技巧：借助伪代码精准控制 LLM 的输出"
date: "2026-05-07"
source: "微信公众号"
---

> **摘要** — 当自然语言难以精准控制复杂任务输出时，伪代码提供了一种介于自然语言与编程语言之间的形式化描述方法。本文通过 JSON 格式化、字幕整理、多图生成三个实战案例，展示如何借助变量类型、函数、控制流精准约束 LLM 的输出结构和执行逻辑。核心洞见：Prompt 本质是对 LLM 的控制指令，形式不限于自然语言，伪代码可精准定义执行流程。
...
graph TD
    ...
    C --> D[定义变量类型]
    C --> F[定义函数逻辑]
    C --> G[控制流 if-for-while]
    D --> H[精准输出结果]
    F --> H
    G --> H
    H --> I[JSON格式化|字幕整理|多图生成]
```
...
## 总结
- Prompt 本质是控制指令
- 形式可灵活多样：few-shot / CoT / 伪代码
```

---

# Prompt 高级技巧：借助伪代码精准控制 LLM 的输出
...
![整理 WWDC 访问文稿](../images/pseudocode-ctrl-0.png)

![多个 Speaker，显示发言人](../images/pseudocode-ctrl-1.png)

![1 个 Speaker，不显示发言人](../images/pseudocode-ctrl-2.png)
