...
> **摘要** — 谷歌 68 页提示词白皮书由 Gemini 团队 AI 工程师 Lee Boonstra 撰写（谷歌 8 年经验）。核心内容是 CTF（Clear Thinking Framework）黄金公式 + 元提示词模板。元提示词即"写提示词的提示词"，是一份检查清单指导如何写出好的提示词。

```mermaid
graph TD
    A[谷歌68页提示词圣经] --> B[CTF黄金公式]
    A --> C[元提示词模板]
    A --> D[六大策略]
    D --> D1[写清晰说明]
    D --> D2[给模型角色]
