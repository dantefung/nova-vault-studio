...
> **摘要** — OpenAI 官方提示词指南的六种核心策略：写清晰说明（包含细节、采用角色、使用分隔符、指定步骤、提供示例、指定长度）、提供参考文本（使用参考文本回答、引用）、拆分复杂任务（意图分类、总结对话、递归总结）、给模型时间思考（自己找解决方案、内心独白、询问遗漏）、使用外部工具（嵌入搜索、代码执行、函数调用）、系统测试变化（评估模型输出）。

```mermaid
graph TD
    A[OpenAI六种策略] --> B[写清晰说明]
    A --> C[提供参考文本]
    A --> D[拆分复杂任务]
    A --> E[给模型时间思考]
    A --> F[使用外部工具]
