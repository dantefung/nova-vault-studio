...
#### 3.4.1 ⚠️ 常见问题：不会自动切换 JDK

创建了 `.sdkmanrc` 后，如果进入/离开目录时不自动切换 JDK，通常是 `sdkman_auto_env` 没有开启。

**检查方法**：

```bash
grep auto_env ~/.sdkman/etc/config
```
...
│      项目          │              状态               │
├────────────────────┼─────────────────────────────────┤
│ JDK 21 安装        │ ✅ 21.0.9-tem                   │
├────────────────────┼─────────────────────────────────┤
│ .sdkmanrc 文件     │ ✅ 在项目根目录 java=21.0.9-tem │
├────────────────────┼─────────────────────────────────┤
│ 当前 Java 版本     │ ✅ 21.0.9                      │
└────────────────────┴─────────────────────────────────┘
```

虽然 JDK 和 `.sdkmanrc` 都正确，但 `sdkman_auto_env=false` 时，**离开/进入项目目录不会自动切换**。
