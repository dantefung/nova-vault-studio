...
建知识库时，切分是最先做、最没技术含量、也最容易被随便糊弄过去的一步。但它**决定了检索的最小单位**——切错了，后面所有环节都在补救。
...
这类错误特别危险，因为 _它是自信的、有出处的、而且只在特定情况下出现_。测试时很难覆盖到。

**切得太大**：一整章塞进一段。[一段文字压成一个坐标](https://pmaker.space/learn/embedding.html)，等于把里面所有主题平均了一下——退货、换货、发票、客诉全混在一个坐标里，哪个主题都不突出，结果是 _什么问题都能召回一点，什么问题都不够准_。而且这一大段塞进上下文，[中间部分很可能被忽略](https://pmaker.space/learn/context-rot.html)。
...
关于长度，给个起点：**多数场景下每段 200 到 500 个 [Token](https://pmaker.space/learn/token.html) 比较合适。**但这真的要看内容——法律条款可以短，技术教程往往需要更长才说得完整。**别信任何一个「标准值」，拿你自己的文档试。**
...
为什么有用？因为切出来的片段常常是「无主」的——正文里写着「本条不适用于以下情形」，但「本条」是哪条，信息在标题里，而标题被切走了。[算向量时](https://pmaker.space/learn/embedding.html)，这段孤零零的文字既不知道在讲什么主题，检索时自然也匹配不上。

拼上路径之后，这段文字自带了主题信息，向量位置立刻准确多了。而且模型拿到材料时也能看出它出自哪，[标注出处](https://pmaker.space/learn/rag.html)更方便。

同理，还应该给每段存上**元数据**：来源文档、章节、生效日期、版本、可见范围。这些在检索时能用来过滤——比如只查当前有效的版本，或者只查这个用户有权限看的内容。[过期版本被召回](https://pmaker.space/learn/similarity.html)这个坑，就靠它来堵。

切完之后还有一个五分钟的检查，比调任何参数都有价值：**随机抽二十段出来自己读一遍**，问自己单看这一段能不能明白它在说什么、信息完不完整。
