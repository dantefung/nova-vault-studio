...
于是问到一个它没见过的东西时，它不会停下来。词表里总有概率最高的那个 token，抽出来接上，再接下一个。几十步之后，一段完全成立的假话就写好了。**句子的流畅度和内容的真实性，是两个完全独立的东西，而它只优化前者。**
...
所以这件事没有「修好」的那一天。**能做的是把发生概率压低、把发生之后的代价控住**，不是等一个不会幻觉的模型。
...
第一条的效果远超其余，因为它把任务从「回忆」换成了「阅读理解」——后者它强得多。而**把校验的责任放在你的系统里，不要放在用户的警惕心上**：提示「AI 可能出错，请自行核实」是最省事也最无效的做法。
