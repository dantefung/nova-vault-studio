...
产品结构图不是前两张的简单叠加，中间隔着一次真正的产品设计。它的价值在于成本低：改一个功能的位置只要动一个节点，不用重画原型。

信息结构图最容易画错的地方是**按页面罗列信息**。它应该以对象为单位鸟瞰全局——同一个用户对象的信息会出现在很多页面上，图上只画一次。
...
功能结构图有两条硬约束：模块数量控制在五到九个、层级控制在两到三级。超出的话不是图画错了，是功能范围该收了。
...
三者最容易混的是结构图和架构图。一个说「有什么」，一个说「怎么连、数据往哪流」。前者是产品经理画的，后者通常是开发画的。
