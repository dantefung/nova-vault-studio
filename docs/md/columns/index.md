---
title: "专栏总览"
date: "2026-05-20"
source: "原创"
url: ""
---

# 专栏总览

> 系统化的深度知识专栏，按领域组织，持续更新。

---

## Agentic Engineer

> 聚焦 AI Agent 工程的架构设计、技能系统、项目记忆与自主增强能力建设。

[进入专栏 →](/md/columns/agentic-engineer/)

---

## OpenCode

> 聚焦 OpenCode 生态的智能体编排、团队架构、配置规范与自动化工程实战。

[进入专栏 →](/md/columns/agentic-engineer/opencode/)

---

## Vibe Coding 指南

> Vibe Coding 开发规范、提示词技巧、上下文管理和工作流架构。

[进入专栏 →](/md/columns/vibe-coding/)

---

## Harness Engineering

> 驭化工程完整知识体系：上下文工程、架构约束、熵管理、Agent 可读性。

[进入专栏 →](/md/columns/harness-engineering/)

---

## Lenny's Newsletter

> Lenny Rachitsky 的 Newsletter 与 Podcast 精华合集，产品管理领域最具影响力的内容。

[进入专栏 →](/md/columns/lenny-newsletter/)

---

## 绘图指南

> SVG 绘图、UML 图绘制、AI 生成图片、架构图制作、Drawio 使用、绘图工具合集。

[进入专栏 →](/md/columns/drawing/)

---

## OPC 一人公司

> 一人公司、超级个体、超级岗位、FDE——AI 时代的个体崛起与职业转型。

[进入专栏 →](/md/columns/opc/)

---

## Indie Hub 独立开发者生存手册

> 独立开发者、出海产品、SaaS 商业化——从 0 到 1 的完整路径。覆盖套利模式、AI 跨境电商、SEO、独立 App 实战、一件代发等内容。

[进入专栏 →](/md/columns/indie-hub/)

---

## 提示词从入门到精通

> 基于提示词工程专栏 32 篇文章提炼的完整书籍：方法框架、推理模式、输出控制、反模式避坑、李继刚的 Lisp 之路。

[进入专栏 →](/md/columns/prompt-engineering-book/)

---

## 认知专栏

> 探索程序员的底层思维能力，提升认知水平和问题解决能力。

[进入专栏 →](/md/columns/cognition/)

---

## 智能招聘

> AI 驱动的 HR 工作流自动化：简历筛选、人才库管理、面试调度、评分校准全链路。

[进入专栏 →](/md/columns/recruitment/)

---

## 各种虾

> 虾系 AI 工具集 —— QwenPaw 等各类智能体框架的安装、配置、使用与实践。

[进入专栏 →](/md/columns/paws/)

---

## The Software Architecture Chronicles

> 软件架构编年史：从架构思想到设计模式的系统梳理。基于 Herberto Graca 的同名系列文章。

[进入专栏 →](/md/columns/software-architecture-chronicles/)

---

## 社媒

> X/Twitter 运营、SEO 策略、谷歌搜索控制台实战记录。

[进入专栏 →](/md/columns/social-media/)

---

## 人生健康专栏

> 像调优软件那样调优人体系统——神经科学/营养学/睡眠医学的前沿研究转化为可执行日常实践，覆盖睡眠/饮食/Fasting/多巴胺/学习/大脑健康/长寿七大模块。

[进入专栏 →](/md/columns/healthy-life/)
