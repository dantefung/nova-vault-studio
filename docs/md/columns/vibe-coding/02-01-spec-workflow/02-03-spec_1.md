...
流程：resolve-library-id → get-library-docs触发场景：框架 API、配置文档、版本差异、迁移指南限制参数：tokens≤5000, topic 指定聚焦范围

3. Sequential Thinking（复杂规划）

触发场景：多步骤任务分解、架构设计、问题诊断流程输出要求：生成6到10 步可执行计划，不暴露推理过程参数控制：total_thoughts≤10, 每步一句话描述

4. DuckDuckGo（外部信息）

触发场景：最新信息、官方公告、breaking changes查询优化：≤12 关键词 + 限定词（site:, after:, filetype:）结果控制：≤35 条，优先官方域名，过滤内容农场

5. Playwright（浏览器自动化）

触发场景：网页截图、表单测试、SPA 交互验证安全限制：仅开发测试用途
