...
## 2026-08-03: 主题控件与编辑部文档体验收敛

* 知识库模式采用克制分段式主题控件，博客模式采用编辑部文字式控件，两者共享同一主题状态。
* Easton 正文、代码块、引用、表格、翻页和移动端设置面板统一为直角或 `2px` 小圆角。
* 修复代码块 `pre` 与 `code` 双重 padding 导致的横向溢出，滚动职责统一归 `pre`。
* 移动端隐藏重复的桌面主题控件，只保留搜索与设置入口；首页、博客列表和文档页共用设置面板。
* 修复博客主题点击“开始阅读”后 `ReadingTime.vue` 访问不存在的 `useData().source` 导致正文无法渲染。
* 详细过程、技能、失败尝试和验证证据见[主题控件与编辑部文档体验开发日志](../dev-log/theme-controls-editorial-docs-2026-08-03.md)。
