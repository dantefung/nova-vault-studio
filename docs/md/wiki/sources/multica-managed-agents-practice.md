...
# 人机 Agent 团队协同：从 Managed Agents 原理到 Multica 实践

> 单体 Agent 普遍存在架构耦合、运维成本高、无法团队协作、能力沉淀复用难等痛点。Managed Agents 将智能体决策逻辑与底层运行基础设施解耦，而 Multica 打通了托管理论到研发落地的全链路。
...
![架构与实践插图](../images/multica-managed-agents-practice/001.png)
...
访问[Multica — Project Management for Human + Agent Teams](https://link.csdn.net/?target=https%3A%2F%2Fmultica.ai%2F "Multica — Project Management for Human + Agent Teams")注册账号，创建工作区并进入看板；
...
*   客户端方式：访问[Download Multica | Multica](https://link.csdn.net/?target=https%3A%2F%2Fmultica.ai%2Fdownload "Download Multica | Multica")选择对应系统版本下载安装即可。
...
*   前端：[http://localhost:3000](https://link.csdn.net/?target=http%3A%2F%2Flocalhost%3A3000%2F "http://localhost:3000")
*   后端：[http://localhost:8080](https://link.csdn.net/?target=http%3A%2F%2Flocalhost%3A8080%2F "http://localhost:8080")

**步骤二**：首次登录 + 创建工作区

访问[http://localhost:3000](https://link.csdn.net/?target=http%3A%2F%2Flocalhost%3A3000%2F "http://localhost:3000")，执行如下操作：
...
![架构与实践插图](../images/multica-managed-agents-practice/002.png)

*   Multica Agent：Agent角色的设置类信息。

墨子为例，设置信息包括：运行时（本地MacMini + OpenCode） + Agent 模型（DeepSeek） + 技能（design-taste-frontend） + 指令等。

![架构与实践插图](../images/multica-managed-agents-practice/003.png)

Multica 运行时：我的电脑上安装了 Claude、Codex 和 OpenCode，如下图全部识别可用。

![架构与实践插图](../images/multica-managed-agents-practice/004.png)

#### 4.2、团队协同规则

团队协同规则通过Squad指令来承载，Squad指令会在 Leader 智能体处理分配给该小队的 issue 时注入到它的 [prompt](https://link.csdn.net/?target=https%3A%2F%2Fedu.csdn.net%2Fcourse%2Fdetail%2F41282%3Futm_source%3D2019755004) 中。用来给 Leader 提供贯穿全队的指导、协作规范，或每次任务都应遵循的上下文。
...
![架构与实践插图](../images/multica-managed-agents-practice/005.png)
...
![架构与实践插图](../images/multica-managed-agents-practice/006.png)

**Multica处理核心步骤图例**：

*   需求分析 & 分配"技术方案"任务：「萧何（队长）」 -> 「张良（架构师）」

![架构与实践插图](../images/multica-managed-agents-practice/007.png)

*   技术方案完成：「张良（架构师）」 -> 「萧何（队长）」

![架构与实践插图](../images/multica-managed-agents-practice/008.png)
