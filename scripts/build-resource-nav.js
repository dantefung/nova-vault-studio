...
const DEFAULT_SOURCE = 'docs/md/guide/ai/ai-programming-resources.md'
...
function outputName(source) {
  ...
  const outBase = base.replace(/-resources$/, '-nav')
  // 历史命名：ai-programming-resources.md 同时输出 resource-nav.{js,json} 别名，
  // 保留给 docs/.vitepress/theme/composables/useResourceNav.js 这种早期组件引用。
  const isLegacy = base === 'ai-programming-resources'
  const legacyJs = isLegacy ? path.join(OUTPUT_DIR, 'resource-nav.js') : null
  const legacyJson = isLegacy ? path.join(OUTPUT_DIR, 'resource-nav.json') : null
  return {
    json: path.join(OUTPUT_DIR, `${outBase}.json`),
    js: path.join(OUTPUT_DIR, `${outBase}.js`),
    legacyJs,
    legacyJson,
    source,
  }
...
function processOne(source, checkMode) {
  const { json: outJson, js: outJs, legacyJs, legacyJson } = outputName(source)
  ...
  const jsContent = `// Auto-generated by scripts/build-resource-nav.js. Do not edit.\nexport default ${JSON.stringify(data, null, 2)};\n`
  fs.writeFileSync(outJs, jsContent, 'utf8')
  if (legacyJson) fs.writeFileSync(legacyJson, JSON.stringify(data, null, 2), 'utf8')
  if (legacyJs) fs.writeFileSync(legacyJs, jsContent, 'utf8')

  console.log(
    `[resource-nav] ${path.basename(outJson)}: ${data.stats.categories} categories, ${data.stats.items} items`,
  )
  return true
}
...
function main() {
  ...
  if (sourceArg !== -1 && process.argv[sourceArg + 1]) {
    sources = [path.join(CWD, process.argv[sourceArg + 1])]
  } else if (allMode) {
    sources = [
      path.join(CWD, 'docs/md/guide/ai/ai-programming-resources.md'),
      path.join(CWD, 'docs/md/guide/ai/claude-code-resources.md'),
    ]
  } else {
    sources = [path.join(CWD, DEFAULT_SOURCE)]
  }

  let allOk = true
  for (const src of sources) {
    if (!processOne(src, checkMode)) allOk = false
  }
